...
function UploadSlot({
  ...
  // Task 10: stronger contrast in light mode
  const emptyBg = isDark ? 'rgba(255,255,255,0.02)' : 'rgba(0,0,0,0.04)';
  const emptyBorder = isDark ? 'rgba(255,255,255,0.12)' : 'rgba(0,0,0,0.2)';
  const emptyText = isDark ? 'rgba(255,255,255,0.5)' : 'rgba(0,0,0,0.65)';
  const emptySubtext = isDark ? 'rgba(255,255,255,0.25)' : 'rgba(0,0,0,0.48)';
      ...
      {large ? (
        <>
          ...
          <div className="text-center px-3">
            <p className="text-xs font-medium" style={{ color: emptyText }}>Drop image of the core challenge</p>
            <p className="text-xs mt-0.5" style={{ color: emptySubtext }}>PNG, JPG, JPEG — analyzed by AI vision</p>
          </div>
        </>
      ) : (
        <>
...
export default function LeftPanel() {
  ...
  // Task 10: All colors strengthened for light mode visibility
  ...
  const headerColor = isDark ? 'rgba(255,255,255,0.9)' : 'rgba(0,0,0,0.88)';
  const labelColor = isDark ? 'rgba(255,255,255,0.85)' : 'rgba(0,0,0,0.82)';
  const sublabelColor = isDark ? 'rgba(255,255,255,0.35)' : 'rgba(0,0,0,0.58)';
  const recentTextColor = isDark ? 'rgba(255,255,255,0.7)' : 'rgba(0,0,0,0.78)';
  const recentSubColor = isDark ? 'rgba(255,255,255,0.3)' : 'rgba(0,0,0,0.5)';
  const recentHoverBg = isDark ? 'rgba(255,255,255,0.03)' : 'rgba(0,0,0,0.05)';
  const clockColor = isDark ? 'rgba(255,255,255,0.35)' : 'rgba(0,0,0,0.5)';
  const recentLabelColor = isDark ? 'rgba(255,255,255,0.4)' : 'rgba(0,0,0,0.55)';
  ...
  const aiVisionBg = isDark ? 'rgba(0,220,255,0.05)' : 'rgba(0,136,170,0.07)';
  const aiVisionBorder = isDark ? 'rgba(0,220,255,0.18)' : 'rgba(0,136,170,0.25)';
  const aiVisionDot = isDark ? '#00dcff' : '#0077aa';
  const aiVisionTitle = isDark ? 'rgba(0,220,255,0.9)' : '#005577';
  const aiVisionText = isDark ? 'rgba(255,255,255,0.45)' : 'rgba(0,0,0,0.6)';
  const textareaColor = isDark ? 'rgba(255,255,255,0.85)' : 'rgba(0,0,0,0.82)';
  const textareaBg = isDark ? 'rgba(0,0,0,0.3)' : 'rgba(255,255,255,0.9)';
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', borderBottom: `1px solid ${panelBorder}`, flexShrink: 0 }}>
          ...
          >
            <svg width="14" height="14" viewBox="0 0 14 14" fill="none">
              <polyline points="10,2 4,7 10,12" stroke={isDark ? 'rgba(255,255,255,0.75)' : 'rgba(0,0,0,0.65)'} strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
            </svg>
          </button>
        </div>
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '12px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{
            ...
            border: `1px solid ${problemDescription.trim().length === 0
              ? isDark ? 'rgba(0,220,255,0.35)' : 'rgba(0,136,170,0.45)'
              : isDark ? 'rgba(0,220,255,0.18)' : 'rgba(0,136,170,0.22)'}`,
            ...
            <div style={{ position: 'relative' }}>
              <div style={{ position: 'absolute', top: 10, left: 10, pointerEvents: 'none', color: isDark ? 'rgba(0,220,255,0.4)' : 'rgba(0,136,170,0.5)' }}>
              ...
              <textarea
                ...
                style={{
                  ...
                  border: `1px solid ${problemDescription.trim().length === 0
                    ? isDark ? 'rgba(0,220,255,0.3)' : 'rgba(0,136,170,0.4)'
                    : isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.14)'}`,
                  color: textareaColor,
                ...
                onFocus={e => {
                  e.currentTarget.style.border = `1px solid ${isDark ? 'rgba(0,220,255,0.5)' : 'rgba(0,136,170,0.6)'}`;
                  e.currentTarget.style.boxShadow = isDark ? '0 0 0 2px rgba(0,220,255,0.08)' : '0 0 0 2px rgba(0,136,170,0.1)';
                }}
                onBlur={e => {
                  e.currentTarget.style.border = problemDescription.trim().length === 0
                    ? `1px solid ${isDark ? 'rgba(0,220,255,0.3)' : 'rgba(0,136,170,0.4)'}`
                    : `1px solid ${isDark ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.14)'}`;
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginTop: 5, paddingLeft: 2, paddingRight: 2 }}>
                {problemDescription.trim().length === 0 ? (
                  <span style={{ fontSize: 9.5, color: isDark ? 'rgba(0,220,255,0.6)' : 'rgba(0,136,170,0.8)', display: 'flex', alignItems: 'center', gap: 4 }}>
                    <AlertCircle size={10} /> Required to start growth
                  </span>
                ) : (
                  <span style={{ fontSize: 9.5, color: isDark ? 'rgba(57,255,20,0.6)' : '#1a7700' }}>✓ Ready</span>
                )}
                <span style={{ fontSize: 9.5, color: remaining < 50 ? 'rgba(255,160,50,0.9)' : isDark ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.4)' }}>
                  {remaining}/{charLimit}
                </span>
              </div>
            </div>
          ...
          <div style={{
            borderRadius: 12, padding: '12px',
            background: isDark ? 'rgba(255,100,50,0.04)' : 'rgba(255,100,50,0.06)',
            border: isDark ? '1px solid rgba(255,100,50,0.15)' : '1px solid rgba(220,80,30,0.25)',
          }}>
          ...
          <div style={{
            borderRadius: 12, padding: '12px',
            background: isDark ? 'rgba(0,255,255,0.03)' : 'rgba(0,136,170,0.05)',
            border: isDark ? '1px solid rgba(0,255,255,0.12)' : '1px solid rgba(0,136,170,0.2)',
          }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 10 }}>
          ...
          {/* AI Vision Status */}
          {activeUploadCount > 0 && (
            <div style={{ borderRadius: 10, padding: '10px 12px', background: aiVisionBg, border: `1px solid ${aiVisionBorder}` }}>
          ...
          <div>
            ...
            {['Traffic Flow Analysis', 'Biomimetic Architecture', 'Neural Commerce'].map((name, i) => (
              <div key={i}
                style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '8px', borderRadius: 8, marginBottom: 4, cursor: 'pointer', border: '1px solid transparent', transition: 'background 0.15s' }}
                onMouseEnter={e => (e.currentTarget as HTMLElement).style.background = recentHoverBg}
                onMouseLeave={e => (e.currentTarget as HTMLElement).style.background = 'transparent'}
              >
                <div style={{ width: 32, height: 32, borderRadius: 8, flexShrink: 0, background: `linear-gradient(135deg, ${['rgba(0,255,255,0.2)', 'rgba(57,255,20,0.2)', 'rgba(255,16,240,0.2)'][i]}, rgba(0,0,0,0.3))`, border: `1px solid ${['rgba(0,255,255,0.3)', 'rgba(57,255,20,0.3)', 'rgba(255,16,240,0.3)'][i]}` }} />
