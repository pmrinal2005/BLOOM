...
export default function Canvas({
  ...
  const handleResetView = useCallback(() => {
    setCanvasOffset({ x: 0, y: 0 }); setCanvasZoom(1);
  ...
  const btnBase: React.CSSProperties = {
    ...
    border: isDark ? '1.5px solid rgba(255,255,255,0.22)' : '1.5px solid rgba(0,0,0,0.15)',
    background: isDark ? 'rgba(255,255,255,0.1)' : 'rgba(255,255,255,0.85)',
    color: isDark ? 'rgba(255,255,255,0.82)' : 'rgba(0,0,0,0.65)',
    boxShadow: isDark
      ? '0 2px 10px rgba(0,0,0,0.45), inset 0 1px 0 rgba(255,255,255,0.1)'
      : '0 2px 10px rgba(0,0,0,0.12), inset 0 1px 0 rgba(255,255,255,0.8)',
  };
  const btnActive: React.CSSProperties = {
    ...btnBase,
    background: isDark ? 'rgba(0,220,255,0.2)' : 'rgba(0,160,200,0.15)',
  ...
  return (
    <div className="relative w-full h-full overflow-hidden" style={{ background: canvasBg, transition: 'background 0.3s ease' }}>
      ...
      <div style={{
        ...
        backgroundImage: `radial-gradient(circle, ${dotColor} 1.8px, transparent 1.8px)`,
        backgroundSize: '32px 32px',
      }} />
      <div style={{
        position: 'absolute', inset: 0, pointerEvents: 'none', zIndex: 0,
        backgroundImage: isDark
          ? 'radial-gradient(ellipse at 50% 50%, rgba(0,80,120,0.09) 0%, transparent 70%)'
          : 'radial-gradient(ellipse at 50% 50%, rgba(0,80,180,0.05) 0%, transparent 70%)',
      }} />
      ...
      <svg
        ...
        style={{
          ...
          cursor: draggingConnection ? 'crosshair' : isPanning.current ? 'grabbing' : 'grab',
        }}
      ...
      >
        ...
        {!hasContent && !isLoading && !isError && generationStatus === 'idle' && (
          <g>
            ...
            <text x="50%" y="35%" dy="22" textAnchor="middle" style={{ fill: textOverlayColor, fontSize: 14, fontFamily: 'Inter, sans-serif' }}>
              then click Start Growth to begin.
            </text>
          </g>
        )}
        {isLoading && (
          <text x="50%" y="20%" textAnchor="middle" style={{ fill: 'rgba(0,220,255,0.6)', fontSize: 13, fontFamily: 'Inter, sans-serif' }}>
            Growing garden...
          </text>
        )}
        {isEmpty && (
          <g>
            ...
            <text x="50%" y="38%" dy="20" textAnchor="middle" style={{ fill: textOverlayColor, fontSize: 11, fontFamily: 'Inter, sans-serif' }}>Try different inspirations or adjust sliders.</text>
          </g>
        )}
        {isError && (
          <text x="50%" y="38%" textAnchor="middle" style={{ fill: 'rgba(255,100,100,0.8)', fontSize: 13, fontFamily: 'Inter, sans-serif', fontWeight: 500 }}>
      ...
      {rebalancing && (
        <div className="absolute inset-0 flex items-center justify-center rebalancing-overlay"
          ...
          <div className="px-8 py-5 rounded-2xl text-center"
            style={{ background: isDark ? 'rgba(9,13,24,0.96)' : 'rgba(255,255,255,0.96)', border: '1px solid rgba(0,220,255,0.25)', boxShadow: '0 0 40px rgba(0,220,255,0.1)' }}>
            ...
            <p className="text-xs mt-1" style={{ color: isDark ? 'rgba(255,255,255,0.4)' : 'rgba(0,0,0,0.5)' }}>Redistributing conceptual relationships</p>
          </div>
        </div>
      )}
      ...
      <div style={{
        ...
        <button onClick={() => setCompactView(!compactView)} title="Compact View"
        ...
        <div style={{
          ...
          border: `1.5px solid ${zoomInputFocused ? 'rgba(0,220,255,0.5)' : isDark ? 'rgba(255,255,255,0.2)' : 'rgba(0,0,0,0.15)'}`,
          display: 'flex', alignItems: 'center', justifyContent: 'center',
          ...
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <input type="text" inputMode="numeric" value={zoomInputValue}
              ...
              onKeyDown={e => {
                ...
                else if (e.key === 'Escape') { setZoomInputValue(String(Math.round(canvasZoom * 100))); (e.target as HTMLInputElement).blur(); }
              }}
              style={{
                width: zoomInputValue.length > 2 ? 22 : 18,
                background: 'transparent', border: 'none', outline: 'none',
                textAlign: 'right', fontSize: 9, fontWeight: 700,
                color: zoomInputFocused ? 'rgba(0,220,255,0.95)' : isDark ? 'rgba(255,255,255,0.65)' : 'rgba(0,0,0,0.65)',
                cursor: 'text', padding: 0, fontFamily: 'Inter, sans-serif',
              } as React.CSSProperties}
              maxLength={3}
            />
            <span style={{
              fontSize: 9, fontWeight: 700,
              color: zoomInputFocused ? 'rgba(0,220,255,0.75)' : isDark ? 'rgba(255,255,255,0.45)' : 'rgba(0,0,0,0.45)',
              userSelect: 'none',
            }}>%</span>
          </div>
        </div>
